...
public enum VkBufferUsageFlags {
    ...
    public static long getMaskOf(Iterable<VkBufferUsageFlags> bits) {
        if (bits == null) return 0;
    ...
    public static boolean doesMaskHave(long mask, VkBufferUsageFlags bit) {
        if (bit == null) return false;
        return (mask & bit.bit) != 0;
    }

    public static Set<VkBufferUsageFlags> maskAsSet(long mask) {
        Set<VkBufferUsageFlags> set = new HashSet<>();
        for (VkBufferUsageFlags bit : values()) {
            if ((bit.bit & mask) != 0) set.add(bit);
        }
        return set;
    }

    public static VkBufferUsageFlags valueOf(long bitVal) {
        for (VkBufferUsageFlags bit : values()) {
